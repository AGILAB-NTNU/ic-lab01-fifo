...
32x32 tiles for hardware tiling verification.
"""
...
def run() -> None:
    ...
    # 矩陣維度: M=64, K=32, N=64 (變數全小寫符合 PEP 8 N806 規範)
    ...
    # 3. 輸出 Hex 檔案 (使用 with 語句確保檔案正確關閉，依 4 塊 Tile 依序寫出)
    ...
    with (
        open(path_a, "w", encoding="utf-8") as fa,
        open(path_b, "w", encoding="utf-8") as fb,
        open(path_c, "w", encoding="utf-8") as fc,
    ):
